...
static const std::unordered_map<GUI::Entities::RessourcesType, std::string>
idOfRessources = {{GUI::Entities::RessourcesType::FOOD, "_FOOD"},
...
"src/GUI/assets/environment/rocks.png"},
{GUI::Entities::RessourcesType::DERAUMERE,
"src/GUI/assets/environment/flower.png"},
{GUI::Entities::RessourcesType::SIBUR, "src/GUI/assets/environment/rose.png"},
{GUI::Entities::RessourcesType::MENDIANE,
"src/GUI/assets/environment/hibiscus.png"},
{GUI::Entities::RessourcesType::PHIRAS,
"src/GUI/assets/environment/camellia.png"},
{GUI::Entities::RessourcesType::THYSTAME,
"src/GUI/assets/environment/dahlia.png"}};

static const std::size_t ressourceLayer = 20;
static const std::string darkFloorPath =
"src/GUI/assets/environment/darkFloor.png";
static const std::string lightFloorPath =
"src/GUI/assets/environment/lightFloor.png";

namespace GUI {
    namespace Entities {

        Floor::Floor(const std::string &id, const Vector2F &position,
        unsigned int width, unsigned int height, float tileSize)
            : AEntity(id, position, Vector2F(0, 0), Vector2F(1, 1),
            EntityType::ENVIROMENT, EntityOrientation::RIGHT),
        ...
        unsigned int Floor::getRessourceAmount(
        Vector2F tile, RessourcesType ressource)
        {
            float x = tile.x;
            float y = tile.y;

            if (_ressourcesNumber.find(Vector2F(x, y))
            == _ressourcesNumber.end()) {
                return (0);
            }
            ...
            return (_ressourcesNumber.find(Vector2F(x, y))
                    ->second.find(ressource)
                    ->second);
        }
        ...
        void Floor::createRessources(unsigned int x, unsigned int y,
        RessourcesType ressource, unsigned int quantity)
        {
            std::string id = std::to_string(x) + std::to_string(y)
            + idOfRessources.at(ressource);

            for (auto itComponents = _components.begin();
                 itComponents != _components.end(); itComponents++) {
                auto entity =
                std::static_pointer_cast<Components::Sprite>(*itComponents);
                if (entity == nullptr) {
                    continue;
                }
        ...
        void Floor::loadRessourceTexture(RessourcesType ressource)
        {
            ...
            if (search == _ressources.end()) {
                if (tx.loadFromFile(pathToData.find(ressource)->second)
                == false) {
                    return;
                } else {
        ...
        void Floor::createRessource(
        unsigned int x, unsigned int y, RessourcesType ressource)
        {
            std::string id = std::to_string(x) + std::to_string(y)
            + idOfRessources.at(ressource);
            float ressourceSize = computeRessourceSize();
            Vector2F ressourcePos =
            computeRessourcePosition(x, y, ressourceSize);

            loadRessourceTexture(ressource);
            _components.push_back(std::make_shared<GUI::Components::Sprite>(id,
            _ressources.find(ressource)->second, ressourceLayer, ressourcePos,
            ressourceSize, ressourceSize));
        ...
        void Floor::createDarkFloor(unsigned int x, unsigned int y)
        {
            ...
            std::make_shared<GUI::Components::Sprite>(id, _txFloorDark, 10,
            Vector2F(_tileSize * static_cast<float>(x),
        ...
        void Floor::createLightFloor(unsigned int x, unsigned int y)
        {
            ...
            _components.push_back(
            std::make_shared<GUI::Components::Sprite>(id, _txFloorLight, 10,
            Vector2F(_tileSize * static_cast<float>(x),
            _tileSize * static_cast<float>(y)),
            _tileSize, _tileSize));
        }
