...
namespace GUI {
    namespace Entities {
        ...
        AEntity::AEntity(const std::string &id, const Vector2F &position,
        const Vector2F &rotation, const Vector2F &scale, EntityType type,
        EntityOrientation orientation)
            : _position(position), _rotation(rotation), _scale(scale),
              _type(type), _id(id), _orientation(orientation)
        {
        }
